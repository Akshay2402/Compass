...
int main(){
	...
	fx = fopen("c:\\Users\\Akshay Sharma\\Desktop\\replica\\delcolx.txt","r");
	if(fx == NULL){
		printf("File X cannot be opened");
		exit(1);
	}
	
	fy = fopen("c:\\Users\\Akshay Sharma\\Desktop\\replica\\delcoly.txt","r");
	if(fy == NULL){
	...
	while(fscanf(fx,"%f",&x) != EOF && fscanf(fy,"%f",&y) != EOF ){
			...
			printf("%d\tx = %f\ty =%f\tHeading = %d\tDirection = %s\n",index++, x, y,heading,dir);
